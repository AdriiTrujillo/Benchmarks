...
//Boolean_T crc32file(   long *charcnt)
//{    
//}
/*
...
void test()
{
      ...
      {     
            ...
            //char*cp = (char**)malloc(5*sizeof(char*));
            //sprintf(cp, "%08lX", crc);
            errors[i]|= Success_;
            solutions[i] = (char*)malloc(5*sizeof(char*));
            sprintf(solutions[i],"%08lX %7ld", oldcrc32, charcnt);
            i++;
      }
...
void check(){
      for (int i=0; i<2; i++) {
            for(int j= 0; j<sizeof(expected[i]) ;j++){
                  if(expected[i][j]!=solutions[i][j]){
                        total_errors+=1;
            ...
            total_errors = total_errors + errors[i];
      }
